...
flowchart LR
    ...
    SEG --> B[".2 - .9<br/>网络设备预留"]
    SEG --> C[".10 - .19<br/>物理机"]
    SEG --> D[".20 - .39<br/>旧节点上的 VM 与容器"]
    SEG --> E[".40 - .59<br/>新节点上的 VM 与容器"]
    SEG --> F[".60 - .99<br/>全局预留"]
    SEG --> G[".100 - .199<br/>DHCP 动态池"]
    SEG --> H[".200 - .239<br/>预留"]
    SEG --> I[".240 - .250<br/>k8s LoadBalancer"]
    C --> C1["pve-old .10<br/>pve-x99 .11<br/>macmini .12"]
    D --> D1["vm-router .20<br/>ct-gateway .21<br/>ct-data .22<br/>vm-k8s-cp .25"]
    E --> E1["vm-k8s-w1 .40<br/>vm-inference .42<br/>vm-dev .43"]
```

| 范围 | 用途 | 分配方式 |
|---|---|---|
| `.1` | 主路由器 | 固定 |
| `.2 – .9` | 网络设备预留（AP、交换机管理口） | — |
| **`.10 – .19`** | **物理机** | 静态 / DHCP 保留 |
| **`.20 – .29`** | **旧节点上的基础服务** | 静态 |
| **`.30 – .39`** | **旧节点扩展预留** | — |
| **`.40 – .49`** | **新节点上的虚拟机** | 静态 |
| **`.50 – .59`** | **新节点扩展预留** | — |
| `.60 – .99` | 全局预留（练手 VM、临时实例） | — |
| **`.100 – .199`** | **DHCP 动态池**（手机、笔记本、电视） | 路由器自动 |
| `.200 – .239` | 预留 | — |
| **`.240 – .250`** | **k8s LoadBalancer（MetalLB）** | 静态，必须在 DHCP 池外 |
| `.251 – .254` | 临时 / 调试 | — |

**判断规则**：看到 `.2x` / `.3x` 就知道在旧节点上，`.4x` / `.5x` 在新节点上，`.1xx`（三位数）是临时接入的终端设备。不用查表。

### 2.2 具体分配

```
━━━ 物理机 .10 – .19 ━━━━━━━━━━━━━━━━━━━━━
192.168.5.1      中兴光猫              主路由
192.168.5.10     pve-old              旧节点 Proxmox 宿主机
192.168.5.11     pve-x99              新节点 Proxmox 宿主机
192.168.5.12     macmini              Mac mini（常开，救援通道）
192.168.5.13–19  ─ 预留 ─              以后：NAS / IP-KVM / 树莓派

━━━ 旧节点上的 .20 – .39 ━━━━━━━━━━━━━━━━
192.168.5.20     vm-router            软路由（VPN + 代理）
192.168.5.21     ct-gateway           LiteLLM + OpenWebUI
...
━━━ 自动分配 ━━━━━━━━━━━━━━━━━━━━━━━━━
192.168.5.90     vm-test              练手用（预留段，跑通后删）

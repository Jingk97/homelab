...
flowchart TB
    ...
    MAC["Mac mini<br/>.9 有线 / .8 无线<br/>常开, 救援通道"]
...
| `.1` | 主路由器 | 固定 |
| **`.2 – .7`** | **网络基础设施（物理或虚拟）** | 静态 |
| `.8 – .9` | Mac mini（`.9` 有线 / `.8` 无线） | DHCP 保留 |
| **`.10 – .19`** | **Proxmox 宿主机与其他物理机** | 静态 / DHCP 保留 |
| **`.20 – .39`** | **旧节点（常开）上的 VM / LXC**<br/>`.2x` 开发相关 · `.3x` 模型相关 | 静态 |
| **`.40 – .59`** | **新节点（按需开机）上的 VM / LXC** —— 实验性，可重建 | 静态 |
...
192.168.5.8      macmini-wifi         Mac mini 无线（🔴 须先关「专用 Wi-Fi 地址」才能绑）
192.168.5.9      macmini              Mac mini 有线（硬件 MAC 永不变，常年插着）
...
━━━ 旧节点 VM/LXC .20 – .39（常开节点）━━━━
【.2x 开发相关】
192.168.5.21     vm-dev               开发 + AI agent            VMID 121
...
192.168.5.26–29  ─ 预留 ─
【.3x 模型相关】
192.168.5.31     ct-llm               LiteLLM + OpenWebUI（模型网关）
192.168.5.32–39  ─ 预留 ─              向量库 / RAG / 模型缓存

━━━ 新节点 VM/LXC .40 – .59（实验性）━━━━
192.168.5.40     vm-k8s-w1            k8s worker
192.168.5.41     vm-k8s-w2            k8s worker
192.168.5.42     vm-inference         GPU 推理（5060 Ti 直通）
192.168.5.43     ─ 已释放 ─            原规划 vm-dev，已迁到旧节点 .21
                                       🔴 原因：X99 定位为「学习才开机」，
                                       开发机必须落在常开节点上
...
**有线 `.9`、无线 `.8`，两个都在光猫上做了 DHCP 保留。**

| 接口 | 地址 | 前提 |
|---|---|---|
| 有线 | `192.168.5.9` | 硬件 MAC 永不变，直接绑 |
| 无线 | `192.168.5.8` | 🔴 **必须先关「专用 Wi-Fi 地址」** |

关闭随机 MAC：`系统设置 → Wi-Fi → 当前网络右侧 ⓘ → 专用 Wi-Fi 地址 → 关闭`，
然后**断开重连**让 MAC 变回真实硬件地址，**再去路由器绑定**（顺序不能反）。
不关就绑不住 —— 随机 MAC 每次连接都变，保留规则永远匹配不上。

> **🔴 更正一条早期的错误判断。** 本节原先写着「实测 `.9` 和 `.100` 的端口
> 指纹完全一致（6152/6153 Surge、5900 屏幕共享），说明是同一台机器的有线
> 与 Wi-Fi」——**这个结论是错的**。
>
> `.100` 实测是 **MacBook Pro**（`sysctl -n hw.model` 返回 `MacBookPro18,3`），
> 不是 Mac mini 的无线口。指纹一致的真正原因是**两台 Mac 都装了 Surge、
> 都开了屏幕共享** —— 同型号服务的端口指纹本来就一样，不能据此判定是同一
> 台机器。
>
> **方法论**：端口指纹只能证明「跑着同样的服务」，要判定是不是同一台机器，
> 得看硬件型号（`hw.model`）或 MAC 的 OUI 段。
...
192.168.5.21    vm-dev
192.168.5.22    ct-data
192.168.5.23    ct-observability
192.168.5.25    vm-k8s-cp
192.168.5.31    ct-llm
192.168.5.40    vm-k8s-w1
192.168.5.41    vm-k8s-w2
192.168.5.42    vm-inference
192.168.5.60    vm-media
```

之后浏览器直接输 `https://pve:8006`，SSH 直接 `ssh root@vm-k8s-cp`。

> 🔴 名字用 `pve` 不是 `pve-old` —— 机器自身 hostname 就是 `pve`，早期文档的 `pve-old` 已统一。
